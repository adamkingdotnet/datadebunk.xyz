...
def generate_crime_plot():
    ...
    fig.add_trace(
        go.Scatter(x=year, y=incidents, name='Data', line=dict(color='royalblue'))
    )
    ...
    fig.add_trace(
        go.Scatter(x=year, y=[mean_data] * len(year), name='Average', line=dict(color='red', dash='dash'))
    )

    fig.add_trace(
        go.Scatter(x=year, y=incidents + std_data, line=dict(width=0), showlegend=False)
    )

    fig.add_trace(
        go.Scatter(x=year, y=incidents - std_data, name='Std. Dev.', line=dict(width=0),
    ...
    fig.add_trace(
        go.Scatter(x=year, y=trendline, name='Trendline', line=dict(color='green', dash='dot'))
    )
    ...
    latest_significance = "significant" if abs(incidents.iloc[-1] - incidents.iloc[-2]) > std_data else "not significant"
